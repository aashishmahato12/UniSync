...
export default function Calendar({ events, onEvent, updateCalendar, createEvent }: {
  ...
  const eventCard = (event: EventItem, compact = false, index = 0) => <article
    ...
    style={{ '--calendar-delay': `${.5 + Math.min(index, 7) * .08}s`, '--calendar-mobile-delay': `${.46 + Math.min(index, 7) * .08}s` } as CSSProperties}
  >
    ...
    <div className="uni-cal-layout">
      <section className="uni-cal-main">
        ...
        {view === 'month' ? <>
          <div className="uni-cal-grid">
            {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((day, index) => <span className="uni-cal-weekday" key={day} style={{ '--calendar-delay': `${.17 + index * .025}s`, '--calendar-mobile-delay': `${.15 + index * .025}s` } as CSSProperties}>{day}</span>)}
            ...
            {Array.from({ length: days }, (_, i) => {
              ...
              return <button key={value} className={`uni-cal-date ${selected === value ? 'is-selected' : ''} ${today === value ? 'is-today' : ''}`} style={{ '--calendar-delay': `${.24 + i * .012}s`, '--calendar-mobile-delay': `${.22 + i * .012}s` } as CSSProperties} onClick={() => setSelected(value)} onDoubleClick={() => choose(value)} aria-pressed={selected === value} aria-label={`${formatDate(value, { month:'long', day:'numeric' })}, ${hits.length} events`}><span>{i + 1}</span>{hits.length > 0 && <i className={`uni-cal-dot uni-cal-${tone(hits[0])}`} />}</button>
            })}
          </div>
      ...
      <aside className="uni-cal-side">
        ...
        {upcoming.length ? <div className="uni-cal-upcoming-list">{upcoming.slice(0, showAll ? undefined : 5).map((event, index) => {
          ...
          return <button className={`uni-cal-upcoming uni-cal-${tone(event)}`} key={`${filter}-${event.id}`} onClick={() => choose(event.date)} style={{ '--calendar-delay': `${.55 + Math.min(index, 7) * .08}s`, '--calendar-mobile-delay': `${.51 + Math.min(index, 7) * .08}s` } as CSSProperties}>
