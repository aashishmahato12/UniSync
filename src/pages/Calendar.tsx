...
export default function Calendar({ events, onEvent, updateCalendar, createEvent }: {
  ...
  return <div className={`uni-calendar uni-calendar-${view}`}>
    ...
    <div className="uni-cal-layout">
      <motion.section className="uni-cal-main" initial={reduceMotion ? false : { opacity: 0, y: 13, scaleX: .96, scaleY: 1.04 }} animate={{ opacity: 1, y: 0, scaleX: 1, scaleY: 1 }} transition={reduceMotion ? { duration: 0 } : calendarEntry(2)}>
      ...
      <motion.aside className="uni-cal-side" initial={reduceMotion ? false : { opacity: 0, y: 13, scaleX: .96, scaleY: 1.04 }} animate={{ opacity: 1, y: 0, scaleX: 1, scaleY: 1 }} transition={reduceMotion ? { duration: 0 } : calendarEntry(3)}>
        ...
        {upcoming.length ? <div className="uni-cal-upcoming-list">{upcoming.slice(0, showAll ? undefined : 5).map((event, index) => {
          ...
          return <motion.button className={`uni-cal-upcoming uni-cal-${tone(event)}`} key={`${filter}-${event.id}`} onClick={() => choose(event.date)} initial={reduceMotion ? false : { opacity: .2, y: 11, scaleX: .93, scaleY: 1.07 }} animate={{ opacity: 1, y: 0, scaleX: 1, scaleY: 1 }} transition={reduceMotion ? { duration: 0 } : calendarEntry(index + 4)}>
