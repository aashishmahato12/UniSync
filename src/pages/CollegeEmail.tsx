  ...
  return <div className="college-email-page">
    ...
    <div className="college-email-layout">
      <section className="college-email-compose">
        ...
        <div className="college-email-compose-foot"><span>{message.length.toLocaleString()} / 10,000</span><button type="button" onClick={() => void send()} disabled={busy || !canSend}>{!canSend ? 'Email not connected' : busy ? 'Queuing…' : <>Send email <Send size={15} /></>}</button></div>
      ...
      <aside className="college-email-side">
        ...
        <p>{canSend ? 'Emails are queued privately, then sent by your connected Gmail workflow.' : 'Your account needs its own email connection before it can send.'}</p>
