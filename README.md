...
UniSync uses verified email sign-in and private accounts. Apply [`n8n/009_private_self_signup.sql`](./n8n/009_private_self_signup.sql) for account isolation; see the [sign-up setup guide](./n8n/AUTH_SELF_SIGNUP_SETUP.md). In Supabase Authentication → URL Configuration, set the Site URL to `https://unisync-dun.vercel.app` and add `https://unisync-dun.vercel.app/**` to Redirect URLs. Add your local Vite URL if testing sign-in locally.

To let each student connect their own Gmail inbox and send college email, follow the [connected-mail setup guide](./n8n/USER_MAIL_SETUP.md). This requires the `010` migration, a Google OAuth web client, server-only Vercel secrets, and two n8n workflows. The existing original-account workflows remain separate.
